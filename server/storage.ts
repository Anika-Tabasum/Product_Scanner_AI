...
import { userCredits, creditPackages, payments, creditUsage, paymentMethods, type UserCredits, type CreditPackage, type Payment, type CreditUsage, type PaymentMethod } from "@shared/payment-schema";
import { db } from "./db";
...
export interface IStorage {
  ...
  updateUserCredits(userId: number, amount: number): Promise<void>;
  ...
  updateUserVerificationToken(id: number, verificationToken: string): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updatePaymentStatus(paymentId: string | number, status: string, verifiedBy?: number): Promise<void> {
  ...
  constructor() {
    this.sessionStore = new PostgresSessionStore({
      pool,
      createTableIfMissing: false, // Changed to false to prevent table recreation
  ...
  async updateUserVerificationToken(id: number, verificationToken: string): Promise<void> {
    await db
      .update(users)
